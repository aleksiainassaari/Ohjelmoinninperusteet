...
def raportti_aikavali(alkupaiva: datetime.date, loppupaiva: datetime.date, tietokanta: list) -> str:
    ...
    * Aikavälin keskilämpötila (esim. kaikkien tuntien lämpötilojen keskiarvo)
    ...
    """
        raportti = "-----------------------------------------------------\n"
        raportti += f"Raportti väliltä {alkupaiva.day}.{alkupaiva.month}.{alkupaiva.year}-"
        raportti += f"{loppupaiva.day}.{loppupaiva.month}.{loppupaiva.year}\n"
        kulutus = 0
        tuotanto = 0
        lampotila = 0
        ...
        for paiva in tietokanta:
            if alkupaiva <= paiva[0].date() <= loppupaiva:
                kulutus += paiva[1]
                tuotanto += paiva[2]
                lampotila += paiva[3]
        ...
        raportti += (
        "- Keskilämpötila: "
        + f"{(lampotila/((loppupaiva - alkupaiva).days*24)):.2f}".replace(".", ",")
        + " °C\n"
    )
        raportti += "-----------------------------------------------------\n"
        return raportti 
...
def raportti_kk(kuukausi: int, tietokanta: list) -> str:
    ...
    * Kuukauden keskimääräinen vuorokauden lämpötila
    ...
    # Käydään tietokanta läpi ja kerätään tiedot
    for paiva in tietokanta:
    ...
    if paivien_lkm > 0:
        raportti += "- Kokonaiskulutus: " + f"{kulutus:.2f}".replace(".", ",") + " kWh\n"
        raportti += "- Kokonaistuotanto: " + f"{tuotanto:.2f}".replace(".", ",") + " kWh\n"
        raportti += "- Keskilämpötila: " + f"{(lampotila/(paivien_lkm*24)):.2f}".replace(".", ",") + " °C\n"
        raportti += "---------------------------------------------\n"
    else:
        raportti += "Ei tietoja valitulta kuukaudelta.\n"
...
def valikot(paavalikko: bool, alavalikko: bool)-> list:
    ...
    Parametrit:
     ...
     palautus:
        valikon_valinnat (str): valikon valinnat yhdistettyinä arvoihin
    """
...
def main():
     """
     ohjelman pääfunktio: kysyy käyttäjältä inputteja ja tulostaa/vie tiedostoon raportin"""
